      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-end justify-content-start">
          <div class="col-md-12  text-center mb-5">
          	<p class="breadcrumbs mb-0"><span class="mr-3"><a href="/"><?=$home?> <i class="ion-ios-arrow-forward"></i></a></span> <span><?=$browsejobs?></span></p>
            <h1 class="mb-3 bread"><?=$browsejobs?></h1>
          </div>
        </div>
      </div>
    </div>
...
<span class="subadge">'.$row["job_type"].'</span>
...
<div><span class="icon-my_location"></span> <span><a class="mr-2" href="Countries.php?search='.$row['name_en'].'">'.$row["name"].'</a>  , <a class="ml-2"  href="cities.php?search='.$row['city_name_en'].'">'.$row["city_name"].'</a></span><br>
</span></div>
<span>'.$row['date'].'  -  '.$row['add_date'].'</span><a href="https://www.blog.jobsagent.org/p1.php?job='.$row['id'].'" class=" small ml-3 ">Send To WP</a><br>
...
<a href="/readmore.php?job='.$row['id'].'" class="btn btn-primary m-2">'.$read_more.'</a>
</div>
</div>';
...
<h3 class="heading-sidebar"><?=$search_by_jobtitle?></h3>
...
{	
  if($language_ch="ar")
{
  echo'<option class="small" value='.$row["id"].'>'.$row["job"].'</option>';

}
else

{
  echo'<option class="small" value='.$row["id"].'>'.$row["job_en"].'</option>';
...
$sql = "SELECT * from  countries order by native desc";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
while($row = $result->fetch_assoc())
{	
echo'<option class="small" onChange="get_city()"  value='.$row["id"].'>'.$row["native"].'</option>';
}
} else {
echo "0 results";
}
