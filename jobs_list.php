...
<?php
echo'<div class="row mr-5 ml-5 p-2 bordered">';

echo $row['job_type_en'].'	 | 	'.$row['company'] ;
echo'<br>';
echo $row['job_cat_en'].' 	| 	'.$row['name_en'].','.$row['city_name_en'].'<br>' ;
echo $row['job_title'].'<br>';
echo '<a href="'.$my_url.'/readmore.php?job='.$row['id'].'">'.$my_url.'/readmore.php?job='.$row['id'].'</a>';
...
// Output the hashtag
echo '<a class="small" href="'.$my_url.'/readmore.php?job='.$row['id'].'">'.$hashtags.'</a>  '; // Outputs: #Hello_World_Hows_it_going
echo'#jobs_agent #جوبز_ايجنت<br> 
...
function convertToHashtags($text) {
    ...
    // Add a hashtag symbol to each word
    $hashtags = array_map(function($word) {
        return '#' . $word;
    }, $words);

    // Return the hashtags as a string separated by spaces
