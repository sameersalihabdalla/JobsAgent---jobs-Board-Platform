...
$query = "SELECT * FROM myjobs WHERE name_en LIKE '%$skeywords%' order by name_en desc LIMIT 20";
...
<html lang="<?=$language_ch?>">
  <head>
    <title> <?=$site_name_c?> - <?=$Country?></title>
    <meta charset="utf-8">
      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-end justify-content-start">
          <div class="col-md-12   text-center mb-5">
          	<p class="breadcrumbs mb-0"><span class="mr-3"><a href="/"><?=$home?> <i class="ion-ios-arrow-forward"></i></a></span> <span><?=$Country?></span></p>
            <h1 class="mb-3 bread"><?=$Country?></h1>
          </div>
        </div>
      </div>
    </div>
...
$query = "SELECT * FROM myjobs WHERE name_en LIKE '%$search%' order by name_en desc LIMIT $start, $limit";
$result = $conn->query($query);

$total_query = "SELECT COUNT(*) FROM myjobs WHERE name_en LIKE '%$search%'";
